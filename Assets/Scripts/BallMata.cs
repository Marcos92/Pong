...
public class BallMata : Ball
{
    ...
    public PongCircularMove lastHit;
    public GameManager2 gManager;
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        else if (col.gameObject.tag == "Player")
        {
            ...
            PongCircularMove playerHit = col.gameObject.GetComponent<PongCircularMove>();

            if (playerHit.isMid)
            {
                if (lastHit != null)
                {
                    if (playerHit.team == lastHit.team)
                    {
                        gManager.equipas[playerHit.team].score++;
                    }
                    else
                    {
                        gManager.equipas[playerHit.team].score--;
                    }
                    gManager.UpdateScores();
                }
            }
            else
            {
                Debug.Log("Defininiu lasthit");
                lastHit = playerHit;
                if (pongAngle == 0)
                {
